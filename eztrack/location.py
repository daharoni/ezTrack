...
def _opencv_is_headless():
    """Return True if the installed OpenCV was built without GUI (HighGUI) support.

    opencv-python-headless still exports ``cv2.imshow`` (so ``hasattr`` can't tell
    the builds apart) but raises ``cv2.error`` when it is called. The build report
    is the reliable signal: headless builds print ``GUI: NONE``.
    """
...
def LoadAndCrop(video_dict, cropmethod=None, fstfile=False, accept_p_frames=False):
    ...
    print(
        f"dimensions (h x w): {int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))},{int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))}"
    )
    ...
    if video_dict["dsmpl"] < 1:
        frame = cv2.resize(
            frame,
            (int(frame.shape[1] * video_dict["dsmpl"]), int(frame.shape[0] * video_dict["dsmpl"])),
...
def Locate(cap, tracking_params, video_dict, prior=None):
    ...
    if ret:
        ...
        if "mask" in video_dict and video_dict["mask"]["mask"] is not None:
        ...
        if prior is not None and tracking_params["use_window"]:
            ...
            dif_weights[
                slice(ymin if ymin > 0 else 0, ymax), slice(xmin if xmin > 0 else 0, xmax)
            ] = 1
            dif = dif * dif_weights
        ...
        if tracking_params["rmv_wire"]:
            ...
            if krn_violation:
                print(
                    "WARNING: wire_krn too large. Reverting to rmv_wire=False for frame {x}".format(
...
def ROI_plot(video_dict):
    ...
    image.opts(
        ...
        colorbar=True,
        toolbar="below",
        title="No Regions to Draw"
        if nobjects == 0
        else "Draw Regions: " + ", ".join(video_dict["region_names"]),
    )
...
def ROI_Location(video_dict, location):
    ...
    for poly in range(len(video_dict["roi_stream"].data["xs"])):
        ...
        xy = np.column_stack((x, y)).astype(np.int32)  # xy coordinate pairs (cv2 needs int32)
    ...
    # For each frame assess truth of animal being in each ROI. Indexing is
    # positional (numpy arrays) rather than by the 'Frame' label so it does not
    # depend on the dataframe index matching frame numbers under pandas 3.0.
    yi = location["Y"].to_numpy().astype(np.intp)
    xi = location["X"].to_numpy().astype(np.intp)
    ROI_location = {mask: ROI_masks[mask][yi, xi] for mask in ROI_masks}
...
def ROI_linearize(rois, null_name="non_roi"):
    ...
    """
    # For each frame, join the names of the regions the animal is in (in column
    # order), falling back to null_name when it is in none. Building the label in
    # one pass avoids the original's pandas chained assignment, which silently
    # no-ops under Copy-on-Write (pandas >= 2.0 / default in 3.0).
...
def ROI_transitions(regions, include_first=False):
    ...
    """
    # Compare each frame to the previous one. The first frame is filled with its
    # own value so it is never a transition unless `include_first`. Uses .shift /
    # .iloc rather than positional Series indexing (`regions[0]`), which is
    # deprecated for integer-position access in pandas >= 2.0.
...
def Batch_LoadFiles(video_dict):
    ...
    if os.path.isdir(video_dict["dpath"]):
        ...
        raise FileNotFoundError(
            "{path} not found. Check that directory is correct".format(path=video_dict["dpath"])
        )
...
def Batch_Process(video_dict, tracking_params, bin_dict, accept_p_frames=False):
    ...
    for file in video_dict["FileNames"]:
        ...
        location.to_csv(
            os.path.splitext(video_dict["fpath"])[0] + "_LocationOutput.csv", index=False
        )
...
def PlayVideo_ext(video_dict, display_dict, location, crop=None):
    ...
    # PlayVideo_ext opens an external OpenCV GUI window (cv2.imshow). ezTrack pins
    # opencv-python-headless (matching minian), which still exports cv2.imshow but
    # raises at call time; its build reports "GUI: NONE". Fail fast here -- before
    # opening the capture or writing video_output.avi -- with a friendly message.
    if _opencv_is_headless():
        raise RuntimeError(
            "PlayVideo_ext needs a GUI build of OpenCV, but ezTrack installs "
            "opencv-python-headless. Use PlayVideo() to view frames inline in the "
...
def showtrace(video_dict, location, color="red", alpha=0.8, size=3):
    ...
    return (image * poly * points) if video_dict["roi_stream"] is not None else (image * points)
...
def Heatmap(video_dict, location, sigma=None):
    ...
    for frame in range(len(location)):
        Y, X = int(ys[frame]), int(xs[frame])
        heatmap[Y, X] += 1

    sigma = np.mean(heatmap.shape) * 0.05 if sigma is None else sigma
...
def Mask_select(video_dict, fstfile=False):
    ...
    poly.opts(fill_alpha=0.3, active_tools=["poly_draw"])
    points = hv.Points([]).opts(active_tools=["point_draw"], color="red", size=10)
    streams.PointDraw(source=points, num_objects=2)
        ...
        if len(x_ls) > 0:
            ...
            for submask in range(len(x_ls)):
                ...
                xy = np.column_stack((x, y)).astype(
                    np.int32
                )  # xy coordinate pairs (cv2 needs int32)
