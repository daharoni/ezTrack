"""``eztrack notebooks`` subcommands: copy bundled notebooks out of the package."""
...
DEFAULT_DEST = "eztrack-notebooks"


def _cmd_list(args: argparse.Namespace) -> None:  # noqa: ARG001
    print_table([(name, desc) for name, (_file, desc) in NOTEBOOKS.items()])
...
def add_subparser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser("notebooks", help="copy bundled notebooks out of the package")
    ...
    cp.add_argument("-o", "--output", help=f"destination directory (default: ./{DEFAULT_DEST})")
